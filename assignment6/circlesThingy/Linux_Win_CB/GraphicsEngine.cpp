                     ...
                     sf::ContextSettings(24, 8, 4, MajorVer, MinorVer, sf::ContextSettings::Core))
{
    ...
    circleAtts = new CirclesAtts[100];

    for(int i =0; i < 100; i++){
        circleAtts[i].setCenter( (float) (rand() % 21 - 10 ), (float) (rand() % 21 - 10 ), (float)(rand() % 21 - 10 ));
        circleAtts[i].setColor((float)(rand()/RAND_MAX), (float)(rand()/RAND_MAX), (float)(rand()/RAND_MAX), 1);
        circleAtts[i].setRotateAngle((rand()%41)  -20);
        circleAtts[i].setRotateVec((float)( rand() % 5 -2), (float)(rand() % 5 -2), (float)(rand() %5 -2));
    }
...
void GraphicsEngine::display()
{
    ...
    for(int i=0; i < 100; i++){
        GLfloat cen[3], rotVec[3], colors[4];
        circleAtts[i].getCenter(cen);
        circleAtts[i].getRotateVec(rotVec);
        circleAtts[i].getColor(colors);
        glm::mat4 model = glm::translate(glm::mat4(1.0), glm::vec3(cen[0], cen[1], cen[2]) );
        model = glm::rotate(model, mult *  circleAtts[i].getRotateAngle() * degf, glm::vec3(rotVec[0], rotVec[1], rotVec[2]));
        glUniform4fv(ColorLoc, 1, glm::value_ptr(glm::vec4(1.0, 1.0, 1.0, 0.0)));//glm::vec4(colors[0], colors[1], colors[2], colors[3])));
