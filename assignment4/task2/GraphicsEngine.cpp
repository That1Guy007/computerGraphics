...
void GraphicsEngine::initWorld()
{
    ...
    {
        star[i].setCenter(-1.5 + 3*(float)rand()/RAND_MAX, -1.5 + 3*(float)rand()/RAND_MAX);
        float sidelength = (float)rand()/RAND_MAX;
        star[i].setRadius(0.25 + sidelength * 0.50);

        star[i].setPointColor((float)rand()/RAND_MAX, (float)rand()/RAND_MAX, (float)rand()/RAND_MAX);

        world.push_back(&star[i]);
    }
}
